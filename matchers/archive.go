...
const defaultArchivePriority = 10

var (
	TypeEpub   = newType("epub", "application/epub+zip", defaultArchivePriority)
	TypeZip    = newType("zip", "application/zip", defaultArchivePriority)
	TypeTar    = newType("tar", "application/x-tar", defaultArchivePriority)
	TypeRar    = newType("rar", "application/x-rar-compressed", defaultArchivePriority)
	TypeGz     = newType("gz", "application/gzip", defaultArchivePriority)
	TypeBz2    = newType("bz2", "application/x-bzip2", defaultArchivePriority)
	Type7z     = newType("7z", "application/x-7z-compressed", defaultArchivePriority)
	TypeXz     = newType("xz", "application/x-xz", defaultArchivePriority)
	TypePdf    = newType("pdf", "application/pdf", defaultArchivePriority)
	TypeExe    = newType("exe", "application/x-msdownload", defaultArchivePriority)
	TypeSwf    = newType("swf", "application/x-shockwave-flash", defaultArchivePriority)
	TypeRtf    = newType("rtf", "application/rtf", defaultArchivePriority)
	TypeEot    = newType("eot", "application/octet-stream", defaultArchivePriority)
	TypePs     = newType("ps", "application/postscript", defaultArchivePriority)
	TypeSqlite = newType("sqlite", "application/x-sqlite3", defaultArchivePriority)
	TypeNes    = newType("nes", "application/x-nintendo-nes-rom", defaultArchivePriority)
	TypeCrx    = newType("crx", "application/x-google-chrome-extension", defaultArchivePriority)
	TypeCab    = newType("cab", "application/vnd.ms-cab-compressed", defaultArchivePriority)
	TypeDeb    = newType("deb", "application/x-deb", defaultArchivePriority)
	TypeAr     = newType("ar", "application/x-unix-archive", defaultArchivePriority)
	TypeZ      = newType("Z", "application/x-compress", defaultArchivePriority)
	TypeLz     = newType("lz", "application/x-lzip", defaultArchivePriority)
	TypeRpm    = newType("rpm", "application/x-rpm", defaultArchivePriority)
	TypeElf    = newType("elf", "application/x-executable", defaultArchivePriority)
)
